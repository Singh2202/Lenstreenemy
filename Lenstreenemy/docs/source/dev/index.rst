...
The Lenstreenemy project implements a regression
...
not broken existing functionality. The Lenstreenemy
...
.. code-block:: console
    ...
    (Lenstreenemy) $ make test

To run tests verbosely use:

.. code-block:: console

    (Lenstreenemy) $ make test-verbose
...
``Lenstreenemy`` package has been installed. If this is
not the case then you will likely need to set the ``PYTHONPATH`` environment
variable so that the ``Lenstreenemy`` package can be found.

.. code-block:: console

    (Lenstreenemy) $ cd tests
    (Lenstreenemy) $ python -m unittest

Individual unit tests can be run also.

.. code-block:: console

    (Lenstreenemy) $ python -m test_version
...
.. code-block:: console
    ...
    (Lenstreenemy) $ make sort-imports
...
.. code-block:: console
    ...
    (Lenstreenemy) $ make check-static-analysis
...
- Create and push a repo tag to Github. As a convention use the package
  ...
  - This will trigger Github to create a release at:
    ...
    ::
        ...
        https://github.com/{username}/Lenstreenemy/releases/{tag}
...
- Test the release distribution. This involves creating a virtual environment,
  ...
  .. code-block:: console
      ...
      (Lenstreenemy) $ make dist-test

- Upload the release to PyPI using

  .. code-block:: console

      (Lenstreenemy) $ make dist-upload

  The package should now be available at https://pypi.org/project/Lenstreenemy/
